...
import { getTelemetry, getEvents, getSessions, getDrivers } from '../services/api';
        ...
        getSessions(year, gp)
            .then((data) => {
                if (Array.isArray(data)) {
                    setSessions(data);
                    const keys = data.map(s => s.SessionKey).filter(Boolean);
                    if (keys.length > 0) {
                        setSession(prev => keys.includes(prev) ? prev : (keys.includes('Q') ? 'Q' : keys[keys.length - 1]));
                    }
                }
            })
            .catch(console.error);
